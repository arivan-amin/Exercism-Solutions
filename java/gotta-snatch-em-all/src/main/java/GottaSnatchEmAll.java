...
class GottaSnatchEmAll {
    ...
    static boolean canTrade (Set<String> myCollection, Set<String> theirCollection) {
        boolean doesPlayerHaveCardsToTrade =
            myCollection.stream().anyMatch(s -> !theirCollection.contains(s));
        boolean doesFriendHaveCardsToTrade =
    ...
    static Set<String> commonCards (List<Set<String>> collections) {
        ...
        IntStream.range(0, allElements.size()).forEachOrdered(i -> {
    ...
    private static boolean isElementIsCommon (List<Set<String>> collections, int i,
        ...
        return collections.stream().allMatch(set -> set.contains(allElements.get(i)));
    }
