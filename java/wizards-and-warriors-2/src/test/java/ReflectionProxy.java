...
public abstract class ReflectionProxy {
    ...
    public Class<?> getTargetClass () {
        try {
            return forName(getTargetClassName());
        }
     ...
     * @return
     */
    public boolean isMethodReturnType (Class<?> returnType, String name,
                                       Class<?>... parameterTypes) {
        return isMethodReturnType(returnType, null, name, parameterTypes);
    }
    
    /**
     * Checks if a method from the target class returns a correct parameterized collection 
     * (Generics)
     *
     ...
     */
    public boolean isMethodReturnType (Class<?> returnType, Class<?> parameterizedType, String name,
                                       Class<?>... parameterTypes) {
        Class<?> targetClass = getTargetClass();
        if (targetClass == null || name == null) {
            return false;
        ...
        try {
            ...
            if (!(m.getGenericReturnType() instanceof ParameterizedType pType)) {
                return false;
            }
    ...
    /**
     * Checks if an existing Collection type has the parameterized type (Generics) as expected 
     * (eg. List<String>)
     *
    ...
    public boolean isPropertyOfType (String name, Class<?> type, Class<?> parameterizedType) {
        ...
        try {
            ...
            if (!(f.getGenericType() instanceof ParameterizedType pType)) {
                return false;
            }
            return pType.getActualTypeArguments()[0].equals(parameterizedType);
        }
     ...
     */
    protected <T> T invokeMethod (String methodName, Class<?>[] parameterTypes,
    ...
    public boolean extendsClass (String className) {
        ...
        try {
            Class<?> parentClass = forName(className);
